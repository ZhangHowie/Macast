...
function osc_init()
    ...
    ne.content = function ()
        local parts = {}
        if state.video_w and state.video_w > 0 then
            parts[#parts+1] = string.format("%d\195\151%d", state.video_w, state.video_h)
            if state.video_bitrate and state.video_bitrate > 0 then
                parts[#parts+1] = string.format("%.1fMbps", state.video_bitrate / 1e6)
            end
        end
        local cache_state = state.cache_state
        if cache_state and cache_state["seekable-ranges"] and
...
mp.observe_property("video-params", "native", function(name, val)
    if val then
        state.video_w = val.w or 0
        state.video_h = val.h or 0
    else
        state.video_w = 0
        state.video_h = 0
    end
...
mp.observe_property("video-bitrate", "number", function(name, val)
    state.video_bitrate = val or 0
    request_tick()
end)
